...
<template>
  <section id="customer-stories" class="customer-stories" aria-labelledby="customer-stories-title">
    <div class="customer-stories__inner">
      ...
      <p class="customer-stories__notice">
        Демонстрационные сценарии на основе возможностей продукта. Они не являются отзывами реальных клиентов.
      </p>
...
.customer-stories { padding: clamp(64px, 8vw, 116px) max(16px, 2.78vw); color: #111; font-family: var(--font-primary, "TT Commons Pro", sans-serif); background: #f2f2f0; }
.customer-stories__inner { width: min(100%, 1360px); margin: 0 auto; }
.customer-stories__header { display: grid; grid-template-columns: .7fr 1.45fr .85fr; align-items: start; gap: 32px; }
.customer-stories__header p, .customer-stories__header h2, .customer-stories__notice, .customer-stories__panel p, .customer-stories__panel h3, .customer-stories__details, .customer-stories__controls p { margin: 0; }
...
.customer-stories__header h2 { max-width: 760px; font-size: clamp(42px, 5vw, 72px); font-weight: 500; line-height: .98; letter-spacing: -.045em; }
.customer-stories__intro { max-width: 340px; color: #5a5b62; font-size: 18px; line-height: 1.4; }
.customer-stories__notice { max-width: 710px; margin: 32px 0 48px auto; padding-left: 18px; border-left: 2px solid #111; color: #5a5b62; font-size: 15px; line-height: 1.45; }
.customer-stories__layout { display: grid; grid-template-columns: minmax(210px, .58fr) minmax(0, 2.42fr); }
.customer-stories__tabs { display: grid; grid-template-rows: repeat(3, 1fr); border-top: 1px solid #babbb8; }
.customer-stories__tab { display: grid; grid-template-columns: 34px 1fr; align-items: center; gap: 12px; min-height: 92px; padding: 20px; border: 0; border-bottom: 1px solid #babbb8; border-radius: 0 !important; color: #6a6b70; font: inherit; font-size: 17px; text-align: left; background: transparent; cursor: pointer; transition: color 180ms ease, background-color 180ms ease; }
.customer-stories__tab:hover { color: #111; background: #e7e7e3; }
.customer-stories__tab:focus-visible { position: relative; z-index: 1; outline: 2px solid #111; outline-offset: -3px; }
.customer-stories__tab--active { color: #111; background: #fff; }
.customer-stories__tab--active:hover { background: #fff; }
.customer-stories__tab-number { color: #939491; font-size: 13px; }
.customer-stories__panel-shell { min-height: 430px; }
.customer-stories__panel { display: grid; grid-template-columns: minmax(200px, .7fr) minmax(0, 2fr); min-height: 430px; background: #111; color: #fff; }
.customer-stories__identity { display: flex; flex-direction: column; justify-content: space-between; padding: clamp(26px, 3vw, 42px); border-right: 1px solid #3a3a3a; }
.customer-stories__identity p:first-child { font-size: 19px; line-height: 1.25; }
.customer-stories__identity p:last-child { margin-top: 6px; color: #999; font-size: 15px; }
...
.customer-stories__scenario-label { color: var(--story-accent); }
...
.customer-stories__details div { padding-top: 16px; border-top: 1px solid #464646; }
.customer-stories__details dt { margin-bottom: 10px; color: #999; font-size: 14px; text-transform: uppercase; letter-spacing: .06em; }
.customer-stories__details dd { margin: 0; color: #e6e6e6; font-size: 17px; line-height: 1.4; }
.customer-stories__deliverables { display: flex; flex-wrap: wrap; gap: 8px; margin: 28px 0 0; padding: 0; list-style: none; }
.customer-stories__deliverables li { padding: 8px 12px; border: 1px solid #505050; border-radius: 999px; color: #c5c5c5; font-size: 14px; }
.customer-stories__controls { grid-column: 2; display: flex; align-items: center; justify-content: space-between; padding-top: 18px; }
...
@media (max-width: 1023px) {
  .customer-stories__header { grid-template-columns: .65fr 1.35fr; }
  .customer-stories__intro { grid-column: 2; }
  .customer-stories__notice { margin-left: 32.5%; }
  .customer-stories__layout { grid-template-columns: 1fr; }
  .customer-stories__tabs { grid-template-columns: repeat(3, 1fr); grid-template-rows: none; }
  .customer-stories__controls { grid-column: 1; }
}

@media (max-width: 767px) {
  .customer-stories__header { display: block; }
  .customer-stories__label { margin-bottom: 24px !important; }
  .customer-stories__intro { margin-top: 22px !important; font-size: 16px; }
  .customer-stories__notice { margin: 28px 0 36px; }
  .customer-stories__tabs { display: flex; max-width: 100%; overflow-x: auto; scrollbar-width: none; }
  .customer-stories__tabs::-webkit-scrollbar { display: none; }
  .customer-stories__tab { flex: 0 0 176px; min-height: 74px; padding: 16px; }
  .customer-stories__panel-shell { min-height: 660px; }
  .customer-stories__panel { grid-template-columns: 1fr; min-height: 660px; }
  .customer-stories__identity { flex-direction: row; align-items: center; justify-content: flex-start; gap: 18px; padding: 24px 20px; border-right: 0; border-bottom: 1px solid #3a3a3a; }
